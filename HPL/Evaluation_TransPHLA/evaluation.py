...
def zeroshot_data_preparation(pos_dataset_type, batch_size, configs, seq_type, seq_dict, hla2candidates, eval_type):

    if pos_dataset_type == "zeroshot_eg":
        zeroshot_df = pd.read_csv(os.path.join(
            data_path, "zeroshot_set.csv"),
            index_col=0)
    elif pos_dataset_type == "zeroshot_ab":
        zeroshot_df = pd.read_csv(os.path.join(
    ...
    if eval_type == "fullranking":
        return zeroshot_pos_df
    else:
        zeroshot_pos_df[seq_type] = zeroshot_pos_df["HLA"].map(lambda x: seq_dict[x])
        test_dataset_RN = pHLA_Dataset_RN(zeroshot_pos_df, hla2candidates, configs)
        test_loader_RN = torch.utils.data.DataLoader(
            test_dataset_RN,
            batch_size=batch_size,
            shuffle=False,
        ...
        return zeroshot_pos_df, test_loader_RN
...
def full_ranking(
    ...
    with torch.no_grad():
        ...
        # y_prob_softmax_all = torch.cat(y_eval_prob_softmax_list, dim=0).cpu().detach()
        # out_all = torch.cat(output_list, dim=0).cpu().detach()
        # torch.save(y_prob_softmax_all, "./soft_score_all.pt")
        # torch.save(y_prob_all, "./new_score_all.pt")
        # torch.save(out_all, "./out_all.pt")
    
        _, index_of_rank_list = torch.topk(y_prob_all, len(ranking_pool))
        # _, index_of_rank_list = torch.topk(y_prob_softmax_all, len(ranking_pool))

        ordered_pred_peps = [ranking_pool[_] for _ in index_of_rank_list.numpy()]
        positive_ranks = sorted([
            ordered_pred_peps.index(pos_pep) for pos_pep in test_pos_peptides
        ])
        bottom_positive_ranks = positive_ranks[-bottom_k:]

        # print(len(ordered_pred_peps))
        # print(ordered_pred_peps[0])
        ...
        for ind, k in enumerate(topk):
            num_hit = len(test_pos_peptides.intersection(set(ordered_pred_peps[:k])))
...
def full_ranking_test(
    ...
    for current_HLA in HLA_list[:]:
        ...
        if pos_dataset_type == "test":
            train_length_cur_HLA = len(
                train_pos_df[train_pos_df["HLA"] == current_HLA])
    ...
    # results_df.to_csv(f"../PLM/fullranking_csvs/{pos_dataset_type}/"+log_name)
...
if __name__ == "__main__":
    ...
    ## EG
    pos_dataset_type = "zeroshot_eg"
    HLA2ranking_candidates = np.load(
                data_path + "zeroshot_allele2candidate_pools.npy",
                allow_pickle=True,
            ).item()
    (zeroshot_pos_df, test_loader_RN
    )= zeroshot_data_preparation(pos_dataset_type, args.batch_size, dataset_configs, 
                                args.seq_type, hla_seq_dict, HLA2ranking_candidates, eval_type="all")
    # classify-RN
    test_times = 5
    avg_mertric = np.array([0.0] * 9)
    for i in range(test_times):
    ...
    #     HLA2ranking_candidates, args, args.recall_logname+".csv",
    #     pos_dataset_type=pos_dataset_type
    #     )
