...
def full_ranking(
    ...
    hla_max_len, pep_max_len, lengths,
    ...
    with torch.no_grad():
        ...
        for i in lengths:
            ...
            positive_ranks = sorted(
                [recall_peps.index(pos_pep) for pos_pep in select_pos_pep]       # positive peptides' rank
            )
            ...
            for ind, k in enumerate(topk):
                num_hit = len(select_pos_pep.intersection(set(recall_peps[:k])))
                if num_pos != 0:
...
def full_ranking_test(
    ...
    for current_HLA in target_HLA_list:
        ...
        recall_cur_dict, hit_cur_dict = full_ranking(
            ...
            args.hla_max_len, args.pep_max_len, lengths,
    ...
    for i in lengths:
        ...
        if args.fullranking_target == "all":
            results_df.to_csv(
                "../fullranking_csvs/{}/HPL-Pan/{}_len{}.csv".format(
                    fullranking_type, args.fullranking_target, log_name, i))
...
if __name__ == "__main__":
    ...
    if args.eval_options == "all" or args.eval_options ==  "fullranking":
        ...
        t1 = time.time()
        if args.fullranking_target == "single":
            log_name = "{}mer_RN_ft_{}_{}_{}_ep{}_{}{}{}".format(
                args.hla_max_len, plm_type, dir_saver[42:-6], 
                lr, epoch,
                args.target_hla[4], args.target_hla[6:8], args.target_hla[9:]
                )
        else:
            log_name = "{}mer_RN_ft_{}_{}_{}_ep{}".format(
                args.hla_max_len, plm_type, dir_saver[41:-6], 
                lr, epoch
                )
        print(f"\n{log_name}\n")
        full_ranking_test(
